...
function prepareResponse(payload, errors, status, message) {
  return {
    result: {
      errors: errors,
      success: status,
...
var passwordExpired = function (user, passwordTemplate) {
  ...
  if (difference > passwordTemplate.maxDaysAllowedForPasswordExpiry || user.passwordInvalidated == true) return true;
  else return false;
};
...
var forceLogout = function (sessions, sessionData) {
  ...
  sessions.forEach(function (session) {
    if (sessionData.loginMode == 'WEB' || sessionData.loginMode == 'DIRECT') {
      if ((session.loginMode == 'WEB' || session.loginMode == 'DIRECT') && session.status == 'authenticated' && isSessionValid(session)) sameSession = true;
    } else {
      if (session.loginMode == sessionData.loginMode && session.status == 'authenticated' && isSessionValid(session)) sameSession = true;
    }
  });
...
var validateQuestionLength = function (answers) {
  var flag = false;
  answers.forEach(function (answer) {
    if (answer.question.length < config.SECURITY_QUESTION_MINIMUM_LENGTH) flag = true;
  });
  return flag;
};
...
var hasInvalidSpecialCharacters = function (answers, eciperTemplate, type) {
  ...
  if (eciperTemplate.specialCharactersAllowedList) {
    ...
    var regex = config.REGEX_SPECIAL_CHARACTERS_ALLOWED_FOR_SECUIRTY_QUESTION_AND_ANSWERS;
    isInvalid = validatingSpecialCharacters(answers, regex, type, isInvalid);
  }
  return isInvalid;
};
...
const resetPwdHtmlContent = (request) => {
  return `<!DOCTYPE html>
  <html lang="en">
  <head>
  ...
  cursor: pointer;
	...
	<div class="container" >
      <form method="post" action="/api/v1/resetpwd">

      
      <input type="hidden" name="token" value="${request.query.key}" />
      ...
      <input type="submit" value="Reset Password" title="Reset password" onclick="return Validate()" />
      </p>
...
const successHtmlContent = (request) => {
  return `<!DOCTYPE html>
  <html>
  <head>
  <meta name="viewport" content="width=device-width, initial-scale=1">
...
module.exports = {
  ...
  resetPwdHtmlContent: resetPwdHtmlContent,
